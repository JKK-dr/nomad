...
from vint_train.models.future_prediction import compress_sequence_encoding
...
class NoMaD_ViNT(nn.Module):
    ...
    def _encode_single_images(self, img: torch.Tensor) -> torch.Tensor:
        encoding = self.obs_encoder.extract_features(img)
        encoding = self.obs_encoder._avg_pooling(encoding)
        if self.obs_encoder._global_params.include_top:
            encoding = encoding.flatten(start_dim=1)
            encoding = self.obs_encoder._dropout(encoding)
        return self.compress_obs_enc(encoding)
    ...
    def encode_future_images(self, future_img: torch.Tensor) -> torch.Tensor:
        ...
        future_encoding = self._encode_single_images(future_img)
    ...
    def encode_current_obs(self, obs_img: torch.Tensor) -> torch.Tensor:
        current_img = obs_img[:, 3 * self.context_size:, :, :]
        current_encoding = self._encode_single_images(current_img)
        return compress_sequence_encoding(
            current_encoding,
            self.future_encoding_size,
        )
