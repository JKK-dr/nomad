...
class ViNT_Dataset(Dataset):
    def __init__(
        ...
        self.future_spacing = future_prediction.get("spacing", None)
    ...
    def __getitem__(self, i: int) -> Tuple[torch.Tensor]:
        ...
        if self.future_prediction_enabled:
            ...
            future_image = torch.cat([
                self._load_image(f_curr, t) for t in future_times
            ])
        ...
        if self.future_prediction_enabled:
            data = data + (torch.as_tensor(future_image, dtype=torch.float32),)
        return data
