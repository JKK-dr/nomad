...
def _model_has_future_prediction(model: nn.Module) -> bool:
    return getattr(_unwrap_model(model), "future_prediction_enabled", False)
...
def _unpack_batch(data):
    ...
    future_image = extra[0] if len(extra) > 0 else None
    return (
        obs_image,
        goal_image,
        action_label,
...
def _prepare_future_images(
    future_image: Optional[torch.Tensor],
    transform: transforms,
    device: torch.device,
) -> Optional[torch.Tensor]:
    if future_image is None:
        return None
    future_images = torch.split(future_image, 3, dim=1)
    future_images = [transform(img).to(device) for img in future_images]
    return torch.cat(future_images, dim=1)


def _future_target_encoding(
    model: nn.Module,
    future_image: torch.Tensor,
) -> torch.Tensor:
    model = _unwrap_model(model)
    with torch.no_grad():
        if hasattr(model, "encode_future_images"):
            future_encoding = model.encode_future_images(future_image)
        else:
            future_encoding = model("future_target_encoder", future_img=future_image)
...
def _compute_future_loss(
    model: nn.Module,
    future_image: Optional[torch.Tensor],
    future_encoding_pred: Optional[torch.Tensor],
) -> Optional[torch.Tensor]:
    if future_image is None or future_encoding_pred is None:
        return None
    future_encoding_label = _future_target_encoding(model, future_image)
    assert (
        future_encoding_pred.shape == future_encoding_label.shape
    ), f"{future_encoding_pred.shape} != {future_encoding_label.shape}"
    return F.mse_loss(future_encoding_pred, future_encoding_label)
...
def _compute_losses(
    ...
    future_loss: Optional[torch.Tensor] = None,
    ...
    if future_loss is not None:
        results["future_loss"] = future_loss
...
def train(
    ...
    use_future_prediction = _model_has_future_prediction(model)
    loggers = {
    ...
    if use_future_prediction:
        future_loss_logger = Logger("future_loss", "train", window_size=print_log_freq)
        loggers["future_loss"] = future_loss_logger
    ...
    for i, data in enumerate(tqdm_iter):
        ...
        future_image = _prepare_future_images(future_image, transform, device)
        ...
        future_encoding_pred = None
        if use_future_prediction:
            if future_image is None:
                raise ValueError("future_prediction is enabled, but the dataset did not return future images")
            dist_pred, action_pred, future_encoding_pred = model_outputs
        else:
            dist_pred, action_pred = model_outputs[:2]
        future_loss = _compute_future_loss(
            model,
            future_image,
...
def evaluate(
    ...
    if use_future_prediction:
        future_loss_logger = Logger("future_loss", eval_type)
        loggers["future_loss"] = future_loss_logger
...
def _compute_losses_nomad(
    ...
    batch_future_images: Optional[torch.Tensor] = None,
):
    """
    Compute losses for distance and action prediction.
    """
...
def train_nomad(
    ...
    with tqdm.tqdm(dataloader, desc="Train Batch", leave=False) as tepoch:
        for i, data in enumerate(tepoch):
            ...
            batch_future_images = _prepare_future_images(future_image, transform, device)
            action_mask = action_mask.to(device)
            ...
            if i % print_log_freq == 0:
                losses = _compute_losses_nomad(
                            ...
                            batch_future_images=batch_future_images,
                        )
...
def evaluate_nomad(
    ...
    use_future_prediction = _model_has_future_prediction(ema_model)
    loggers = {
        "uc_action_loss": uc_action_loss_logger,
        "uc_action_waypts_cos_sim": uc_action_waypts_cos_sim_logger,
        "uc_multi_action_waypts_cos_sim": uc_multi_action_waypts_cos_sim_logger,
    ...
    if use_future_prediction:
        future_loss_logger = Logger("future_loss", eval_type, window_size=print_log_freq)
        loggers["future_loss"] = future_loss_logger
    num_batches = max(int(num_batches * eval_fraction), 1)
